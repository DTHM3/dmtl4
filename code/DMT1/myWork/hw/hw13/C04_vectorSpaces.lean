  ...
  *\**, to *normal* (fully reduced) forms. Here they
  ...
  instance : Module K Vc :=
  {
    ...
    smul_add := by
      intro a v1 v2
      apply congrArg Vc.mk
      change a * (v1.val + v2.val) = a * v1.val + a * v2.val
      ring1

    add_smul := by
      intro a b v
      apply congrArg Vc.mk
      change (a + b) * v.val = a * v.val + b * v.val
      ring1
  ...
  @@@ -/
  theorem neg_add_cancelVC : ∀ (a : Vc), -a + a = 0 := by
    intro a
    apply congrArg Vc.mk
    simp [Neg.neg, smulKVc, addVc]
    change -1 * a.val + a.val = 0
    ring1

  theorem sub_eq_add_negVc : ∀ (a b : Vc), a - b = a + -b := by
    intro a b
    apply congrArg Vc.mk
    simp [Sub.sub, Neg.neg, smulKVc, addVc]
    change a.val - b.val = a.val + (-1 * b.val)
    ring1
