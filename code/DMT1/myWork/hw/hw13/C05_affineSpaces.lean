...
theorem vsub_vadd'VcPt : ∀ (p₁ p₂ : Pt), (p₁ -ᵥ p₂) +ᵥ p₂ = p₁ := by
  intro p1 p2
  apply congrArg Pt.mk
  simp [VSub.vsub, vsubVcPt, VAdd.vadd, vaddVcPt]

theorem vadd_vsub'VcPt : ∀ (g : Vc) (p : Pt), (g +ᵥ p) -ᵥ p = g := by
  intro g p
  apply congrArg Vc.mk
  change (vaddVcPt g p).val - p.val = g.val
  simp [vaddVcPt]
