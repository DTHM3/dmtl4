...
instance {α : Type u} {n : Nat} [Ring α]: AddGroup (Vc α n) :=
{
  ...
  sub := fun x y => ⟨x.tuple + (- y.tuple)⟩
  sub_eq_add_neg := by
    intros a b
    apply congrArg Vc.mk
    apply congrArg Tuple.mk
    funext
    simp [Sub.sub, Add.add, Neg.neg]
  ...
  neg_add_cancel := by
    ...
    apply congrArg Tuple.mk
    funext
    simp [Add.add, Neg.neg]
}

instance {α : Type u} {n : Nat} [Ring α] [Nonempty (Pt α n)] : AddTorsor (Vc α n) (Pt α n) :=
{
  vadd := fun v p => ⟨v + p⟩,
  vsub := fun p1 p2 => ⟨p1 + (-p2)⟩,
  ...
  vadd_vsub' := by
    ...
    apply congrArg Tuple.mk
    funext i
    simp [Add.add, Sub.sub, Neg.neg]
    exact add_neg_cancel_right (v.tuple i) (p.tuple i),
  ...
  vsub_vadd' := by
    ...
    funext i
    simp [Add.add, Sub.sub, Neg.neg]
    exact neg_add_cancel_right (p.tuple i) (v.tuple i),
  ...
  add_vadd := by
    ...
    funext i
    simp [Add.add]
    exact add_assoc (v1.tuple i) (v2.tuple i) (p.tuple i)
}
