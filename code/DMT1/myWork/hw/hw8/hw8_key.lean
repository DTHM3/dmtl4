...
example : (~(P ⊕ Q)) -> (~P) × (~Q) :=
fun (h : (~(P ⊕ Q))) =>
  (fun p => h (Sum.inl p), fun q => h (Sum.inr q))
...
/-
Cannot be done in the backwards direction as it is
invalid that from (P → Empty) → Empty we can derive P
which cannot be done in constructive logic.
-/
