...
-- ¬(P ∧ Q) → ¬P ∨ ¬Q
example : (P × Q → Empty) → (P → Empty) ⊕ (Q → Empty)
| h =>
  Sum.inl (fun p => h ⟨ p, Q.mk ⟩)  -- Always choosing ¬P constructively Could not get ¬Q to work since its not a proposition


-- ¬(P ∨ Q) → ¬P ∧ ¬Q
example : (P ⊕ Q → Empty) → ((P → Empty) × (Q → Empty)) :=
  fun h => (fun p => h (Sum.inl p), fun q => h (Sum.inr q))

-- ¬(P ∧ N) -- N cannot be proven
example : ~(P × N)
| h => nomatch h

-- (P ∨ N) -- N cannot be proven but P can
example : P ⊕ N := Sum.inl P.mk
...
/-
  Having a proof of ¬¬P means that assuming ¬P leads to a contradiction, but in constructive logic this does not give proof of P itself.
-/
