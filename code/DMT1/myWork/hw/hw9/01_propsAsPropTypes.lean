...
-- ¬(P ∧ Q) → ¬P ∨ ¬Q - Not deriveable for similar reasons to as before with types as it cannot be proved one is false

-- ¬(P ∨ Q) → ¬P ∧ ¬Q
example : ¬(P ∨ Q) → ¬P ∧ ¬Q :=
fun h =>
  let notP : P → False := fun p => h (Or.inl p)
  let notQ : Q → False := fun q => h (Or.inr q)
  And.intro notP notQ

-- ¬(P ∧ N) → Cannot be proven since N is false
example : ¬(P ∧ N) :=
fun ⟨p, n⟩ => nomatch n


-- (P ∨ N)
theorem p_or_n_iff_p : (P ∨ N) ↔ P :=
  Iff.intro
    -- (P ∨ N) → P
    (fun h => h.elim
      (fun p => p)
