...
instance [∀ i Decideable α] : Eq (Typle α n) where
  refl x y := ⟨fun i => rfl⟩
...
def eqRawTuple {α : Type u} (a b: Fin n → α): Prop :=
∀ (i : Fin n), a i = b i

def tupleEq {α : Type u} {n : Nat} (a b : Tuple α n) :=
eqRawTuple a.toFun b.toFun

def vcEq {α : Type u} {n : Nat} (a b : Vc α n) :=
tupleEq a.tuple b.tuple

instance

instance (α : Type u) (n : Nat) [Add α]: AddMonoid (Vc α n) :=
{
  add_assoc :=
    fun a b c => vcEq (a + b + c) (a + (b + c))
